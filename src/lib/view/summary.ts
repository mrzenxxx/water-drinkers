 ...
 *
 * Цифры сверху дашборда обязаны относиться ровно к тому же отрезку, что и
 * график под ними, поэтому и то и другое считается из одного набора событий
 * и одного диапазона — здесь.
 *
 * Дашборд только показывает: ни одно из этих чисел не участвует в расчёте
 ...
 * не по периоду потребления заказа, а по выбранному отрезку дашборда.
 *
 * `daysPresent` работает по полуоткрытому `[from, to)` (§4.1), а границы
 * дашборда включительные, поэтому верхняя двигается на день вперёд.
 */
