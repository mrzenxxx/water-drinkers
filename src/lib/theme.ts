...
/**
 * Следующая тема для кнопки-переключателя.
 *
 * В интерфейсе два варианта, светлая и тёмная: третья кнопка «системная»
 * занимала место в шапке, а объясняла собой меньше, чем занимала. Значение
 * `system` при этом остаётся — оно нужно для первого визита, когда выбора
 * ещё нет и тему диктует система. Первое же нажатие превращает её в явный
 * выбор, и обратно в `system` интерфейс уже не возвращает.
 */
