...
/**
 * Состав участников (§6.7): добавить с учётными данными, изменить ФИО и отдел,
 * перевыпустить логин и пароль, ограничить, исключить, вернуть, выплатить
 * остаток, назначить или снять администратора.
 *
...
export default async function ParticipantsPage(): Promise<ReactNode> {
  const [participants, departments] = await Promise.all([
  ...
  return (
    <section className="flex flex-col gap-6">
      <AddParticipantForm today={today} departments={departments} />
      ...
      <ParticipantsTable participants={participants} today={today} departments={departments} />
    </section>
  );
}
