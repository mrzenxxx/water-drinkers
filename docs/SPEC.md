...
Шапка одна на все экраны участника: логотип, разделы, участник, тема, выход.
Узкий экран (`< md`) отдаёт место значкам:

- разделы уходят в бургер-меню, открывающееся под шапкой;
- имя и роль прячутся, значок участника остаётся и ведёт на профиль;
- слово «Выйти» прячется, значок остаётся.
