...
- **Байты лежат в базе**, отдельной таблицей `announcement_images`. Решение по хранилищу чеков (открытый вопрос №4) этим не предрешается: адрес выдачи ведёт на приложение (`/api/notices/:id/image`), а не на хранилище, ровно как `Receipt.url` (§8). Отдельная таблица нужна потому, что Prisma выбирает все колонки, и `BYTEA` внутри `announcements` означал бы, что список объявлений на каждом экране вычитывает мегабайты, которые ему не нужны.
