...
function run(): boolean {
  ...
  for (const [themeName, selector] of [
    ...
    */
    ...
    /*
      Сплошные пары «текст на непрозрачной заливке». Бейджик чека намеренно
      инвертирован — в тёмной теме белый с тёмным текстом, в светлой наоборот,
      — и стоит в ряду со стеклянными бейджиками, от которых его этим и
      отличают. Подложка у него своя, значит и проверка своя.
    */
    console.log(`\n${themeName} тема · сплошные заливки`);
    for (const [name, background, text, min] of [
      ['бейджик чека', '--foreground', '--background', 4.5],
    ] as const) {
      const ratio = contrast(parseOklch(tokens.get(text)!), parseOklch(tokens.get(background)!));
      const passed = ratio >= min;
      if (!passed) ok = false;
      console.log(
        `  ${passed ? '✓' : '✗'} ${name.padEnd(22)} ${ratio.toFixed(2)}:1 (нужно ${min}:1)`,
      );
    }
